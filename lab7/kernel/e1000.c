...
e1000_transmit(struct mbuf *m)
{
  ...
  //printf("transmit");
  int index; // TDT寄存器
  struct tx_desc* tx_dp; // TDT寄存器索引指向的描述符的指针
  struct mbuf* tx_mbuf; 

  acquire(&e1000_lock);
  index = regs[E1000_TDT];//读取E1000_TDT控制寄存器
  tx_dp = &tx_ring[index];//访问tx_ring获取描述符指针

  
  if ((tx_dp->status & E1000_TXD_STAT_DD) == 0) {//此时还未完成之前的传输工作，返回-1
    release(&e1000_lock);
    return -1;
  }


  tx_mbuf = tx_mbufs[index];//获取该指针对应的数据包缓冲区，并进行释放
  if (tx_mbuf) {
    mbuffree(tx_mbuf);
  }
  ...
  index = (index + 1) % TX_RING_SIZE;//加一取模来遍历tx环，同时将该索引存放在regs[E1000_TDT]中
...
e1000_recv(void)
{
  ...
  while (1) {
    ...
    //申请一个mbuf,同时将其数据指针（m->head）编程到rx_ring描述符中。将描述符的状态位清除为零。
